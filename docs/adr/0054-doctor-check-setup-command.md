...
- Phase 3 completion requires doctor coverage for setup-critical boundaries: local API runtime state, current extension-host environment, public route `/health`, authenticated `/ready`, stale or mismatched runtime proof, Codex auth, Cursor setup confirmation/traffic/staleness, Harness Routing Workaround decision drift, Quick Tunnel stopped/changed/temporary warnings, local API key rotation repair guidance, Usage Statistics storage degradation, OpenAI-key repair capability/status as non-blocking, non-loopback bind-host warnings, and redaction of secrets/prompts/provider payloads.
- Phase 3 doctor does not need rich quota diagnostics, support bundles, multi-account checks, full diagnostic recording UI, or deep private Cursor internals beyond OpenAI-key repair status.
